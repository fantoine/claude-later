...
server.tool('later_push', 'Add a deferred action to the later-queue. Use this when the user wants to save something for later without interrupting the current task.', {
...
server.tool('later_pop', 'Retrieve and remove the next action from the later-queue (FIFO). Use this when the user is ready to process a deferred action.', {
    cwd: z.string().optional().describe('Current working directory of the Claude Code session'),
}, async ({ cwd }) => {
...
server.tool('later_list', 'List all actions currently in the later-queue.', {
...
server.tool('later_pick', 'Retrieve and remove a specific action from the later-queue by its ID.', {
...
server.tool('later_remove', 'Remove a specific action from the later-queue by its ID.', {
...
server.tool('later_clear', 'Remove all actions from the later-queue at once.', {
    cwd: z.string().optional().describe('Current working directory of the Claude Code session'),
}, async ({ cwd }) => {
...
server.prompt('later-add', 'Add a deferred action to the later-queue', { action: z.string().optional().describe('The action to defer') }, ({ action }) => ({
    messages: [{
            role: 'user',
            content: {
                type: 'text',
...
server.prompt('later-list', 'Show all actions in the later-queue', async () => ({
    messages: [{
            role: 'user',
            content: {
                type: 'text',
...
server.prompt('later-next', 'Retrieve and process the next action from the later-queue', async () => ({
    messages: [{
            role: 'user',
            content: {
                type: 'text',
...
server.prompt('later-pick', 'Pick a specific action from the later-queue by ID or position and execute it', { target: z.string().optional().describe('ID or natural language reference (e.g. "the first one", "3rd item")') }, ({ target }) => ({
    messages: [{
            role: 'user',
            content: {
                type: 'text',
...
server.prompt('later-remove', 'Remove one or more specific actions from the later-queue without executing them', { target: z.string().optional().describe('ID, position, or natural language reference to items to remove') }, ({ target }) => ({
    messages: [{
            role: 'user',
            content: {
                type: 'text',
...
server.prompt('later-clear', 'Clear all actions from the later-queue at once', async () => ({
    messages: [{
            role: 'user',
            content: {
                type: 'text',
