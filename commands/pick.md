...
Once the target item is identified, call later_pick with its ID and the cwd parameter to remove it from the queue, then execute the action immediately without asking for confirmation — treat it as if the user had typed it directly.
